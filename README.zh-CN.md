...
- **她长得一直是她。** 一张基准脸锚定所有自拍 —— 跨场景、跨穿搭、跨几个月都是同一个人。出图后端有 7 个可选（Gemini / OpenAI / Seedream / fal.ai / Replicate / 本地 SD WebUI / 任意兼容端点），设一个 key 就会自动识别；选本地 WebUI 的话，她长什么样这件事完全不出你的机器。
...
| 📷 **AI 自拍**，人脸始终一致（**7 家出图后端**，含纯本地） | 🌆 **有生活的一天**（真实城市 + 天气） | ⏰ **主动消息**，被冷落会自动退让 |
