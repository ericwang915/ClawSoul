...
def prefilter_distress(text) -> bool:
    """Cheap gate: does the message look emotionally charged enough to classify?"""
    return bool(_DISTRESS_RE.search(_flatten(text)))
...
# ── AI-transparency notices (first contact + proactive footer) ──────────────
# A one-time, out-of-character disclaimer shown the first time a user talks to
# their companion (the Telegram equivalent of the web in-chat disclaimer), and
# a tiny "generated by AI" footer appended to proactive (companion-initiated)
# messages.
_FIRST_CONTACT = {
...
def ai_note(lang: str | None) -> str:
    """Tiny 'generated by AI' footer for proactive (companion-initiated) messages."""
    return _pick(_AI_NOTE, lang)
