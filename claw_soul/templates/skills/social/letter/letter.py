...
def _letters_dir() -> str:
    ...
    d = os.path.join(str(config.CLAWSOUL_HOME), "context", "letters")
...
def _agent_name() -> str:
    try:
        ...
        return MemoryManager().list_all().get("bot_name", "") or "ClawSoul"
    except Exception:
        return "ClawSoul"
...
def _relationship_age_days() -> int | None:
    ...
    try:
        from claw_soul.core.memory.milestones import MilestoneManager
...
def _recent_sentiment() -> str | None:
    ...
    try:
        from claw_soul.core.memory.emotional_graph import EmotionalGraph
...
def cmd_save(args) -> int:
    ...
    if args.send:
        try:
            from claw_soul.core.tools import send_file
