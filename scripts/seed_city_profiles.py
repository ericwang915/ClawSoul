...
def _c(lat: float, lon: float, tz: str, lang: str, vibe: str = "",
       events: list[dict] | None = None, profile: str = "") -> dict:
    # ``vibe`` = one-line blurb (persona backstory); ``profile`` = the rich
    # ~500-word cultural profile (food/religion/seasons/transit/…) generated by
    # scripts/enrich_city_profiles.py and injected into the daily planner.
    return {"lat": lat, "lon": lon, "timezone": tz, "language": lang,
            "vibe": vibe, "events": events or [], "profile": profile}
