...
dashboard "Tools" tab. Each entry maps a presentable name to:
  * an internal skill (already in ``herandhim/templates/skills/``), OR
...
AuthType = Literal[
    ...
    "shared",        # uses a key from herandhim.json (e.g. photos)
...
CATALOG: list[Tool] = [
    ...
    Tool(
        name="candid",
        display_name="Candid Shots",
        description="\"Look what I saw\" snapshots — animals, scenery, food, fun finds.",
        icon="📸", icon_key="candid",
        auth_type="shared",
        category="Premium",
        accent="#fb7185",
        server_env="ARK_API_KEY",
