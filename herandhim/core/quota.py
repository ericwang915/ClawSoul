...
All keys are opt-in, under ``quota`` in herandhim.json:
...
def _home() -> str:
    return os.path.realpath(str(config.HERANDHIM_HOME))
...
def check_disk(extra_bytes: int = 0) -> str | None:
    ...
    if s["used_bytes"] + extra_bytes >= s["limit_bytes"]:
        return (
            ...
            "Raise quota.diskMb in herandhim.json, or free some space "
