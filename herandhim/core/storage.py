...
Layout
------
``~/.herandhim/context/herandhim.db`` — single SQLite file, WAL mode.
...
def _db_path() -> str:
    return os.path.join(str(config.HERANDHIM_HOME), "context", "herandhim.db")
...
class StorageManager:
    ...
    def instance(cls) -> "StorageManager":
        path = _db_path()  # resolves per-tenant via config.HERANDHIM_HOME
    ...
    def reset_for_tests(cls) -> None:
        """Drop the cached per-path managers (so tests can switch HERANDHIM_HOME)."""
        with cls._lock:
