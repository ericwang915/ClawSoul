...
Sole source: ``<HERANDHIM_HOME>/context/persona/appearance.md``, written by
...
def _profile_dir() -> str:
    return os.path.join(str(config.HERANDHIM_HOME), "context", "profile")
...
def load_appearance() -> str:
    ...
    if not os.path.isfile(path):
        raise AppearanceNotConfigured(
            ...
            "wizard ran and per-tenant HERANDHIM_HOME is correctly bound."
        )
    try:
