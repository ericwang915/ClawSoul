...
Mirrors :mod:`herandhim.core.culture` (per-country holiday calendars):
...
Drop your own ``<CC>.json`` in ``<HERANDHIM_HOME>/data/cities/`` to add or
...
def all_regions_by_country() -> dict[str, list[str]]:
    ...
    dirs = [
        ...
        Path(_config.HERANDHIM_HOME) / "data" / "cities",   # user's wins
    ]
    for d in dirs:
        if not d.is_dir():
            continue
...
#
#   1. <HERANDHIM_HOME>/data/cities/<CC>.json   — yours, wins
#   2. herandhim/data/cities/<CC>.json         — bundled with the package
#
...
def _load_local(cc: str) -> dict | None:
    ...
    from .. import config as _config
    candidates = [
        Path(_config.HERANDHIM_HOME) / "data" / "cities" / f"{cc}.json",
