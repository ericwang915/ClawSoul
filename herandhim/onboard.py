"""
Interactive onboarding wizard for HerAndHim.
...
Writes herandhim.json and generates context/ identity files
...
# herandhim.core.city.  Nothing about specific places is hardcoded here.
...
def _companion_wizard(cfg: dict) -> dict:
    ...
    print()
    print(_c("  ╭──────────────────────────────────────╮", _MAGENTA))
    print(_c("  │   Meet Your HerAndHim — Companion Setup   │", _MAGENTA))
    print(_c("  ╰──────────────────────────────────────╯", _MAGENTA))
    if existing:
...
def _generate_companion_files(choices: dict) -> None:
    ...
    context_dir = str(config.HERANDHIM_HOME / "context")
...
def _generate_soul_file_en(ch: dict, context_dir: str) -> None:
    ...
    non-CN personas."""
    is_female = ch.get("companionGender", "female") == "female"
    comp_name = ch.get("companionName", "HerAndHim")
...
def run_onboard(config_path: str | None = None) -> Path:
    ...
    print()
    print(_c("  ╔══════════════════════════════════════╗", _CYAN))
    print(_c("  ║       HerAndHim — Setup Wizard        ║", _CYAN))
    print(_c("  ╚══════════════════════════════════════╝", _CYAN))
    print()
    ...
    if choices:
        ...
        print(f"    Companion files generated in: {_c(str(config.HERANDHIM_HOME / 'context'), _BOLD)}")
...
def _optional_keys(cfg: dict, express: bool = False) -> None:
    if express:
        ...
        # the dashboard or a re-run of `herandhim onboard`.
        _photo_backend(cfg)
        _telegram_only(cfg)
        return
...
# (fal, replicate, stability, dashscope, custom) lives in herandhim.json —
...
def _channel_keys(cfg: dict) -> None:
    ...
    wa = channels.setdefault("whatsapp", {
        "phoneNumberId": "", "token": "", "verifyToken": "herandhim_verify",
    ...
    wa_verify = input("  WhatsApp Verify Token (default: herandhim_verify): ").strip()
    if wa_verify:
...
def _validate_key(cfg: dict, provider: dict) -> None:
    ...
    try:
        ...
        print(_c("  You can fix this later in herandhim.json or the web dashboard.", _DIM))


def _save_config(cfg: dict, config_path: str | None) -> Path:
    """Write config to disk (defaults to ~/.herandhim/herandhim.json)."""
    if config_path:
        out = Path(config_path)
    else:
        out = config.HERANDHIM_HOME / "herandhim.json"
    out.parent.mkdir(parents=True, exist_ok=True)

    cfg.setdefault("channels", {
        "telegram": {"token": "", "allowedUsers": []},
        "discord": {"token": "", "allowedUsers": [], "allowedChannels": []},
        "whatsapp": {"phoneNumberId": "", "token": "", "verifyToken": "herandhim_verify", "callbackUrl": "", "allowedNumbers": []},
    })
